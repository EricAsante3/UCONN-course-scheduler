...
test('System Objects: Test 7 -- Pre-scheduling -- with Restrictions', () => {
  ...
  expect(output["status"]).toBe(200);
  expect(output["value"]["ECE 2001"][0]["crn"]).toBe("8178");
  ...
  expect(output2["value"]["ECE 2001"].length).toBe(2);
  ...
  expect(output3["value"]["ECE 2001"].length).toBe(1);
  expect(output3["value"]["ECE 2001"][0]["crn"]).toBe("8178");

  Cartinstance.sectionConstraintAdder("ECE 2001", "8178")
  const output4 = Cartinstance.handlePreScheduleProcessing()

  expect(output4["status"]).toBe(200);
  expect(output4["value"]["ECE 2001"][0]["crn"]).toBe("8178");
  expect(output3["value"]["ECE 2001"].length).toBe(1);
