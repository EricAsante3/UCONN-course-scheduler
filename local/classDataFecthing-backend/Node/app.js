...
// Root route
app.get("/", (req, res) => {
  res.send("Welcome to the simple REST backend!");
});

// POST /api/classes
app.post("/GetClasses", async (req, res) => {
    const { Class, Term, Campus } = req.body;

    if (!Class || !Term || !Campus) {
    return res.status(400).json({ error: "Missing required fields: Class, Term, Campus" });
    }
